...
| F2 | **调整模块** | 照搬 Stocky 工作流：建调整单（多行 SKU，扫码/搜索添加）→ 每行旧量/±delta/新量 → 原因（下拉）+ 员工 + 备注（中文）→ 提交写 Shopify → 行级审计记录。列表页可按原因/员工/日期/SKU 筛选，CSV 导出 |
...
1. 原因管理（预置 11 种）+ 员工映射
2. 调整单：建单（扫码/搜索加行、实时显示当前量）→ 应用（写 Shopify，幂等，失败重试/部分失败处理）→ 锁定 → 列表/筛选/CSV
3. 虚拟库存页：设置/清单/一键撤销
