...
- **进度**: 96%
- **一句话**: 每个主要页面和详情已有独立 URL，前进、后退、返回上一级及列表状态恢复逻辑已补齐。
- **分类**: Shopify App
...
- 导航验收：在 Shopify 后台抽查浏览器地址同步、直接打开详情、跨列表来源返回和前进恢复
...
## 🏁 最近完成
- **完整页面路由与历史导航（2026-08-03）**：旧 hash 半路由升级为 App Bridge 兼容的独立路径（如 `/items/123`、`/history/456`、`/adjustments/789`），服务端支持直接打开/刷新详情；统一使用顶栏前进/后退（移除详情区重复返回按钮），由 App 自己保存完整页面栈，并使用 App Bridge 支持的当前窗口导航保持 Shopify 外层地址和 iframe 一致，不再依赖会被重建的 iframe 历史，连续返回和前进均可跨重载恢复；直接打开详情时顶栏返回安全回默认列表；产品/修改记录/调整单/虚拟库存/已删除产品等筛选、排序和页码写入 URL 并可恢复；旧 `#/...` 链接自动迁移；新增导航单测并完成浏览器交互验收
