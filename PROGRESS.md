...
- **进度**: 97%
- **一句话**: 调整单成功执行后可自动将人员、备注、附件清单和全部商品明细发送到 Lark 群。
- **分类**: Shopify App
...
## 🏁 最近完成
- **Lark 群调整通知（2026-08-03）**：调整单只在 Shopify 成功执行并进入 Applied 后通知；包含编号/原因/仓位/人员/账号/时间/总变化/备注/附件清单和每条商品的变体、Barcode、SKU、Before/Change/After；超长整单自动拆分且记住已发部分，重试不会从头重发；通知失败不回滚 Shopify 库存，详情页可手动重试
