...
- **虚拟库存管理（2026-08-03）**：新页面 `/virtual-stock`（手动调整页入口）。按 SKU+仓位汇总所有「Virtual stock adjustment」净额=当前仍挂账的虚拟库存（历史 418 行、净 +534 从未撤销，此前只能靠备注人肉记）。列出挂账量/当前可售/最近设置时间/最近备注；可售低于挂账时标红「已被消耗，需撤销」；勾选后一键生成撤销草稿（按仓位分单，仍需人工确认提交，不直接写 Shopify）
