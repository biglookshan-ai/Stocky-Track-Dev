...
- **状态**: 开发中 · Lark 调整单通知等待群机器人配置验收
- **进度**: 97%
- **一句话**: 调整单成功执行后会自动发送排版清晰的 Lark 富文本卡片，包含备注、全部商品明细、人员与详情入口。
- **分类**: Shopify App
...
## 🏁 最近完成
- **Lark 群调整通知（2026-08-03）**：调整单只在 Shopify 成功执行并进入 Applied 后通知；通知为绿色富文本卡片，标题只显示 `A0003` 短编号，原因和备注置顶，商品和 Before/After 数量加粗，Change 增加为绿色/减少为红色，Barcode 可点击进入该变体的 Shopify 后台页，底部显示记录/经手员工、调整时间和详情按钮；超长整单自动拆分且记住已发部分，通知失败不回滚 Shopify 库存
