...
## 🏁 最近完成
- **Stocky 历史导入上线**：维护工具支持上传 Stocky Adjustments CSV，预检（映射报告：导入/跳过/新建本地商品/员工/仓位/原因分布）→ 确认导入两步式；融合规则=只导入官方流水覆盖起点(2026-01-29)之前避免重复；导入事件标 Historical import、qty_after 显示未提供、不参与趋势重建（无当年销售数据不编造曲线）；幂等可重跑；未执行行(not_adjusted/failed)不入账；# 内部产品自动建为本地商品
