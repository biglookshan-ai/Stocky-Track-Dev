...
## 🏁 最近完成
- **完整页面路由与历史导航（2026-08-03）**：旧 hash 半路由升级为 App Bridge 兼容的独立路径（如 `/items/123`、`/history/456`、`/adjustments/789`），服务端支持直接打开/刷新详情；统一使用顶栏前进/后退（移除详情区重复返回按钮），历史位置跨 Shopify iframe 重载保存，返回后前进可恢复；直接打开详情时顶栏返回安全回默认列表；产品/修改记录/调整单/虚拟库存/已删除产品等筛选、排序和页码写入 URL 并可恢复；旧 `#/...` 链接自动迁移；新增导航单测并完成浏览器交互验收
