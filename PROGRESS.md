...
## 🏁 最近完成
- **Lark 群调整通知（2026-08-03）**：调整单只在 Shopify 成功执行并进入 Applied 后通知；通知升级为绿色富文本卡片，标题只显示 `A0003` 短编号，备注置顶，商品和 Before/After 数量加粗，Change 增加为绿色/减少为红色，底部保留原因、记录/经手员工和详情按钮；超长整单自动拆分且记住已发部分，通知失败不回滚 Shopify 库存
