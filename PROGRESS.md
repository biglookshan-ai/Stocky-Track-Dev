...
## 🏁 最近完成
- **完整页面路由与历史导航（2026-08-03）**：旧 hash 半路由升级为 App Bridge 兼容的独立路径（如 `/items/123`、`/history/456`、`/adjustments/789`），服务端支持直接打开/刷新详情；顶栏新增前进/后退；所有详情「返回上一级」优先回真实来源，直接打开时安全回默认列表；产品/修改记录/调整单/虚拟库存/已删除产品等筛选、排序和页码写入 URL 并可恢复；旧 `#/...` 链接自动迁移；新增导航单测并完成浏览器交互验收
