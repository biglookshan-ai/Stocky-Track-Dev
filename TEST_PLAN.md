...
4. 确认页面只显示「调整明细」和「添加商品」按钮；点击按钮后弹窗出现搜索框，输入测试商品 Barcode 并点击「搜索」，确认结果同时显示商品、Barcode、SKU、Brand 和当前 Available。
5. 搜索一个超过 50 个变体的宽泛词，确认显示总结果数和页码；使用「下一页 / 上一页」切换，并确认跨页勾选不会丢失。
6. 在弹窗跨页勾选两个商品并点击「加入所选」，确认弹窗关闭且两个商品进入「调整明细」；分别点击 `+`、`−` 并输入数量，确认数量框只显示绝对值，Change 和 After 计算正确。
7. 切换只允许增加/减少的 reason，确认不允许的方向被禁用且现有行自动采用正确方向。
8. 核对 Shopify login account；分别选择“实际记录员工”和两个“经手员工”（至少一个本地员工）。
9. 写一段多行 Notes，并分别添加图片、视频和 PDF/普通文件；保存 Draft 后打开详情确认编号格式为 `ADJ-YYYYMM-xxxxx`，三类人员身份正确，可预览图片、打开视频并下载文件。
10. 再进入编辑，确认人员、备注和附件仍然存在，并测试移除一个附件；此阶段 Shopify 库存必须完全不变。
11. 归档这张测试 Draft。
