...
4. 点击详情的「返回上一级」，确认回到刚才的产品列表，并恢复搜索词、筛选条件和第 2 页，而不是跳到其他页面。
5. 点击顶栏「前进」，确认重新进入刚才的商品详情；再用顶栏「返回」确认按真实访问顺序返回。
6. 分别从修改记录、全局搜索、调整单打开商品或其他详情，再点「返回上一级」，确认返回各自的实际来源。
7. 复制一个商品详情、修改记录详情和调整单详情地址，在新标签页直接打开，确认都能进入对应详情；直接打开时的「返回上一级」应回到安全的默认列表。
