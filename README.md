...
On hand 减少才算已出货销量；订单导致的 Available 减少算下单需求，同一订单的
「下单 − 已出货 − 已释放」用于推算待出货。退货取重新入库的订单事件，采购入库只取
`Purchase order received`；内部调拨、人工调整和 App 修正均不计入销量。周均销量、
预计可售天数和补货依据仍只使用真实净出货，同时保留下单需求、周期图表和关联订单/客户明细。
...
超过单次行数上限时自动拆分。进程部署或重启后会从数据库游标
自动续跑；已写入本地的记录长期保留，因此随着应用运行可查询超过 180 天的历史。
重复运行会按 Shopify 变更编号幂等合并，并清理同一变更的实时占位记录。
日常增量会回看最近 30 分钟，以容纳 ShopifyQL 报表相对 Webhook 的延迟。
